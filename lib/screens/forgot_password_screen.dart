      ...
      children: [
        ...
        TextFormField(
          ...
          validator: (value) {
            ...
            if (!RegExp(r'[A-Z]').hasMatch(value)) return 'Use at least one uppercase letter';
            if (!RegExp(r'[a-z]').hasMatch(value)) return 'Use at least one lowercase letter';
            if (!RegExp(r'[0-9]').hasMatch(value)) return 'Use at least one number';
            if (!RegExp(r'[!@#$%^&*(),.?":{}|<>]').hasMatch(value)) return 'Use at least one special character';
            return null;
          },
        ),
        const SizedBox(height: 24),
        ...
        TextFormField(
          ...
          style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: AppColors.inputLabel),
          decoration: _inputDecoration('Confirm your password', Icons.lock_clock_outlined).copyWith(
            suffixIcon: IconButton(
              icon: Icon(_obscureConfirmPassword ? Icons.visibility_off_rounded : Icons.visibility_rounded, color: Colors.grey, size: 20),
